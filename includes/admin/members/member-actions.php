 ...
 * @return void
 */
function rcp_process_delete_member() {

	if ( ! wp_verify_nonce( $_GET['_wpnonce'], 'rcp-delete-member-nonce' ) ) {
		wp_die( __( 'Nonce verification failed.', 'rcp' ), __( 'Error', 'rcp' ), array( 'response' => 403 ) );
	}
	...
	wp_safe_redirect( add_query_arg( 'rcp_message', 'member_deleted', $redirect_url ) );
	exit;

}
add_action( 'rcp_action_delete_member', 'rcp_process_delete_member' );